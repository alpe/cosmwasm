...
mod tests {
    ...
    fn do_addr_canonicalize_reports_invalid_input_back_to_contract() {
        ...
        let res = do_addr_canonicalize::<MA, MS, MQ>(&env, source_ptr1, dest_ptr).unwrap();
        assert_ne!(res, 0);
        let err = String::from_utf8(force_read(&env, res)).unwrap();
        assert_eq!(err, "Input is not valid UTF-8");

        let res = do_addr_canonicalize::<MA, MS, MQ>(&env, source_ptr2, dest_ptr).unwrap();
        assert_ne!(res, 0);
        let err = String::from_utf8(force_read(&env, res)).unwrap();
        assert_eq!(err, "Input is empty");

        let res = do_addr_canonicalize::<MA, MS, MQ>(&env, source_ptr3, dest_ptr).unwrap();
        assert_ne!(res, 0);
        let err = String::from_utf8(force_read(&env, res)).unwrap();
        assert_eq!(err, "Invalid input: human address too long");
    }
    ...
    fn do_addr_canonicalize_fails_for_broken_backend() {
        ...
        let result = do_addr_canonicalize::<MA, MS, MQ>(&env, source_ptr, dest_ptr);
        match result.unwrap_err() {
            VmError::BackendErr {
                source: BackendError::Unknown { msg, .. },
                ..
    ...
    fn do_addr_humanize_works() {
        ...
        let error_ptr = do_addr_humanize::<MA, MS, MQ>(&env, source_ptr, dest_ptr).unwrap();
    ...
    fn do_addr_humanize_reports_invalid_input_back_to_contract() {
        ...
        let res = do_addr_humanize::<MA, MS, MQ>(&env, source_ptr, dest_ptr).unwrap();
        assert_ne!(res, 0);
        let err = String::from_utf8(force_read(&env, res)).unwrap();
    ...
    fn do_addr_humanize_fails_for_broken_backend() {
        ...
        let result = do_addr_humanize::<MA, MS, MQ>(&env, source_ptr, dest_ptr);
        match result.unwrap_err() {
            VmError::BackendErr {
                source: BackendError::Unknown { msg, .. },
                ..
